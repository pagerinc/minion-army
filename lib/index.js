...
const { warnQueuingTime } = require('./utils.js');
...
module.exports = (_manifest, logger) => {
    ...
    const minions = manifest.workers.reduce((minionsByName, worker) => {
        ...
        minion.on('message', (m, meta) => {

            warnQueuingTime(logger, meta);
            eventEmitter.emit('message', worker.config.name, m, meta);
        });
    ...
    const listenResponses = () => {

        Object.values(minions).forEach((minion) => {

            minion.on('response', (r) => eventEmitter.emit('response', minion.options.name, r));
        });
